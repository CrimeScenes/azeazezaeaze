...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        -- Predicts position more accurately based on character velocity and smooth tracking
...
local function isPlayerKnocked(player)
    ...
    return humanoid and humanoid.Health > 0 and humanoid.Health <= 1 
end
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                ...
                if closestPart then
                    if os.clock() - lastClickTime >= 0.05 then
